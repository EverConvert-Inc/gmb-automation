...
import { CheckCircle2, PhoneCall, Plus, XCircle } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { buttonClasses } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { listLsaClients } from "@/lib/queries-lsa";
import { formatRelativeTime } from "@/lib/utils";

export const dynamic = "force-dynamic";
export const metadata: Metadata = { title: "LSA clients" };

export default async function LsaClientsListPage() {
  const clients = await listLsaClients();
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage which businesses appear on the LSA report and how each one
            is linked to Google Ads and CallRail.
          </p>
        </div>
        <div className="flex flex-wrap items-start gap-2">
          <Link href="/lsa" className={buttonClasses("outline")}>
            Back to report
          </Link>
          <Link href="/lsa/clients/new" className={buttonClasses()}>
            <Plus className="mr-2 h-4 w-4" /> Add LSA client
          </Link>
        </div>
      </header>
      ...
      {clients.length === 0 ? (
        <Card className="surface-brand-tint">
          <CardContent className="flex flex-col items-center gap-4 py-14 text-center">
            ...
            <Link href="/lsa/clients/new" className={buttonClasses()}>
              <Plus className="mr-2 h-4 w-4" /> Add the first LSA client
            </Link>
          </CardContent>
        </Card>
      ) : (
