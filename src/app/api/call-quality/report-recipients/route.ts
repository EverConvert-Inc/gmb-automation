...
export async function POST(req: Request) {
  ...
  const existing = await db.query.callQualityReportRecipients.findFirst({
    where: eq(callQualityReportRecipients.email, parsed.email),
  });
  if (existing) {
    return NextResponse.json(
  ...
  const [row] = await db
    .insert(callQualityReportRecipients)
