...
import { yesterdayIso } from "@/lib/lsa-sync";
...
export async function GET(req: Request) {
  ...
  try {
    const result = await sendDailyLsaEmail({ from, to, dryRun });
    ...
    if (dryRun && "pdf" in result) {
      return new NextResponse(new Uint8Array(result.pdf), {
        ...
        headers: {
          ...
          "Content-Disposition": `inline; filename="lsa-report-${to}.pdf"`,
        },
      });
    }
    ...
    console.error("[lsa-email-report] failed:", message);
