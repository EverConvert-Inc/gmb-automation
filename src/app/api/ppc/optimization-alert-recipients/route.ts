...
export async function POST(req: Request) {
  ...
  const existing = await db.query.ppcOptimizationAlertRecipients.findFirst({
    where: eq(ppcOptimizationAlertRecipients.email, parsed.email),
  });
  if (existing) {
    return NextResponse.json(
  ...
  const [row] = await db
    .insert(ppcOptimizationAlertRecipients)
