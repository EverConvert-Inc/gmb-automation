...
import { lsaCallrailTagCategories } from "@/lib/db/schema";
...
export async function GET(
  ...
  const rows = await db.query.lsaCallrailTagCategories.findMany({
...
export async function POST(
  ...
  const [{ nextSortOrder }] = await db
    .select({
      nextSortOrder: sql<number>`coalesce(max(${lsaCallrailTagCategories.sortOrder}), -1) + 1`,
    })
    .from(lsaCallrailTagCategories)
    .where(eq(lsaCallrailTagCategories.lsaClientId, id));

  const [row] = await db
    .insert(lsaCallrailTagCategories)
    .values({ lsaClientId: id, ...parsed, sortOrder: nextSortOrder })
    .returning();
