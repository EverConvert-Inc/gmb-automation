...
export async function POST(req: Request) {
  ...
  const existing = await db.query.lsaClients.findFirst({
    where: eq(lsaClients.slug, parsed.slug),
  });
  if (existing) {
    return NextResponse.json(
      { error: "An LSA client with this slug already exists" },
      { status: 409 },
    );
  }
  const [row] = await db
