...
export function CallQualityReportRecipientsCard({
  initial,
}: {
  initial: Recipient[];
}) {
  ...
  async function add(e: React.FormEvent) {
    ...
    setAddBusy(true);
    try {
      const res = await fetch("/api/call-quality/report-recipients", {
        method: "POST",
  ...
  async function remove(id: string) {
    setRemovingId(id);
    try {
      const res = await fetch(`/api/call-quality/report-recipients/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Call Quality report recipients</CardTitle>
        <CardDescription>
          Email addresses that receive the daily month-to-date Call Quality
          PDF. Changes apply to the next scheduled send.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        >
          <div className="flex-1">
            <Label htmlFor="call-quality-recipient-email">
              Add a recipient
            </Label>
            <Input
              id="call-quality-recipient-email"
              type="email"
              required
              value={email}
