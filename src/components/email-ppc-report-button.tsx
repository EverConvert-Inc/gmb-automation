...
// the operator currently has selected in the page filter. Auth flows
// through the Supabase middleware that already protects /api/ppc/*.
export function EmailPpcReportButton({
  from,
  to,
}: {
  from: string;
  ...
  async function send() {
    ...
    try {
      const res = await fetch("/api/ppc/send-report", {
        method: "POST",
