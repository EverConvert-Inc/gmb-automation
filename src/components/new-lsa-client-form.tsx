...
// Minimal first-step form: just create the LSA client record. Google Ads +
// CallRail linkage happens on the detail page once the record exists.
export function NewLsaClientForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/lsa/clients", {
        method: "POST",
      ...
      router.push(`/lsa/clients/${created.id}`);
    } catch (err) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <p className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-100">
        ...
        step just creates the LSA client record.
      </p>
      <div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={busy || !name || !slug}>
          {busy ? "Creating…" : "Create LSA client"}
        </Button>
      </div>
    </form>
  );
