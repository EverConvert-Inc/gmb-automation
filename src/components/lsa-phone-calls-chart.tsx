...
import type { LsaByDayPoint } from "@/lib/queries-lsa";
...
export function LsaPhoneCallsChart({ byDay, from, to }: Props) {
  ...
  const seriesById = useMemo(() => {
    ...
    for (const p of byDay) {
      const cur = m.get(p.lsaClientId) ?? {
        name: p.lsaClientName,
        values: new Array(dates.length).fill(0),
      };
      const idx = dates.indexOf(p.date);
      if (idx >= 0) cur.values[idx] = (cur.values[idx] ?? 0) + p.phoneCallCount;
      m.set(p.lsaClientId, cur);
    }
    return m;
  }, [byDay, dates]);
  ...
  // LSA clients, which then gets clipped by the wrapper's overflow-hidden.
