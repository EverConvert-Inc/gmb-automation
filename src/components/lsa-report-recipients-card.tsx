...
export type LsaRecipient = { id: string; email: string };

export function LsaReportRecipientsCard({
  initial,
}: {
  initial: LsaRecipient[];
}) {
  const router = useRouter();
  const [recipients, setRecipients] = useState<LsaRecipient[]>(initial);
  ...
  async function add(e: React.FormEvent) {
    ...
    setAddBusy(true);
    try {
      const res = await fetch("/api/lsa/report-recipients", {
        method: "POST",
      ...
      const created = (await res.json()) as LsaRecipient;
      setRecipients((prev) =>
  ...
  async function remove(id: string) {
    setRemovingId(id);
    try {
      const res = await fetch(`/api/lsa/report-recipients/${id}`, {
        method: "DELETE",
      });
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          11:35 UTC (7:35 AM ET). Changes apply to the next scheduled send.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        >
          <div className="flex-1">
            <Label htmlFor="lsa-recipient-email">Add a recipient</Label>
            <Input
              id="lsa-recipient-email"
              type="email"
              required
              value={email}
